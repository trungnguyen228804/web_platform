 ...
 * @author TungDT
 */
class MySQLCore extends QueryPattern {
    ...
    public function __construct($config = ['dbuser' => '', 'dbpassword' => '', 'dbname' => '', 'dbhost' => 'localhost', 'dbport' => 3306, 'encoding' => 'utf8']) {
        $this->dbuser = $config['dbuser'];
        $this->dbpassword = $config['dbpassword'];
        $this->dbname = $config['dbname'];
        list( $this->dbhost, $this->dbport ) = $this->get_host_port($config['dbhost'], $config['dbport']);
    ...
    function connect($dbuser = '', $dbpassword = '', $dbhost = 'localhost', $dbport = false) {
        ...
        if (!$dbuser) {
            ...
            $this->print_error($ezsql_mysqli_str[1]);
            exit();
        } else { // Try to establish the server database handle
            ...
            if ($this->dbh->connect_errno) {
                ...
                $this->print_error($ezsql_mysqli_str[2]);
                exit();
            } else {
                $this->dbuser = $dbuser;
    ...
    function get_host_port($host, $default = false) {
        $port = $default;
        if ($host == 'p:localhost' || $host == 'p:172.31.8.4') {
...
//                $val = 0;
     ...
     */
    function _count($all = true, $increase = false) {
        if ($increase) {
            $this->num_queries++;
            $this->conn_queries++;
        }
